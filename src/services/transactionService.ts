...
export const getFilteredTransactions = async (params: any) => {
    try {
        const res = await api.get("transactions/get-filtered-transactions", { withAuth: true, params: params } as ApiOptions);
        return res.data;
    } catch (error) {
        throw serviceErrorHandler(error);
    }
...
export const getCategoryWiseExpenses = async (params: any) => {
    try {
        const res = await api.get("transactions/get-category-wise-expenses", { withAuth: true, params: params } as ApiOptions);
        return res.data;
    } catch (error) {
        throw serviceErrorHandler(error);
    }
